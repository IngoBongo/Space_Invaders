...
class Invader(Sprite):
	...
	def load_images(self):
		...
		if self.row == 0:
			...
			number = "3"
		return [pygame.image.load("images/invaders/invader" + number + "-" + str(n) + ".png")
		               for n in range(1, 3)]
	...
	def update(self, current_time, stats):
		"""Update position of invader."""
		# Check if its time to move, only move if not exploded.
		if current_time - self.time_of_last_move >= self.settings.invader_move_time and not self.exploded and stats.game_active:
